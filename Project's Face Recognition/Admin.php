<?php 
 ...
 $serverName = "localhost";
    ...
    //   $sql = "SELECT Nome, Cognome, ID FROM Utenti WHERE Utenti.ID='".$verificate."'";
    //   $sqlresult = @$conn->query($sql);
    //   while($riga = $sqlresult->fetch_assoc()) {
    //     if($riga["ID"] == $verificate) {
    //       $DateUsers = fopen($riga["ID"]."_DateUsers.txt", "w") or die("Unable to open file!");
    //       fwrite($DateUsers, $verificate);
		// 	    fclose($DateUsers);
  ...
  $sql = "SELECT Nome, Cognome, Data_di_nascita, Email, Tipo_utente, ID, Utente_attivo, Accesso_utente FROM Utenti";
  $sqlresult = @$conn->query($sql);
?>
