...
$serverName = "localhost";
		...
		if (isset($_POST['Login'])) {
			...
			if(!$exist)
			{
				...
				if($exist){
					...
					if($exist){
						$resCode = 3;
						$existSql = "SELECT ID FROM Utenti WHERE Utente_attivo='".$active_usr."' AND Tipo_utente='".$type_usr."' AND Email='".$email_usr."' AND Password_utente='".$pass_usr."'";
						$existResult = @$conn->query($existSql);
						$exist = $existResult->fetch_assoc();
						$DateUsers = fopen("./DateUsers.txt", "w+") or die("Unable to open file!");
		...
		if (isset($_POST['btnAddUsr'])) {
			...
			$insertSql = "INSERT INTO Utenti (Nome, Cognome, Data_di_nascita, Email, Password_utente, Tipo_utente, Utente_attivo)
			VALUES ('".$name_usr."', '".$surname_usr."', '".$changeDate."', '".$email_usr."', '".$pass_usr."', '".$type_usr."', '".$active_usr."')";
			echo $insertSql."<br>";
