<?php 
	...
	if(isset($_GET["resCode"]))
	{
		...
		if($resCode == 1)
		{
			$message = "Errore nei dati, la partita immessa è già presente";
		}
		if($resCode == 2)
		{
			$message = "Errore nei dati, la partita non è stata inserita";
		}
        if($resCode == 10)
		{
			$message = "Partita rimossa con successo";
		}
	}
	
	$sql = "SELECT Num_partita, Nome_turno, Luogo, Data, Sq_casa, Sq_trasferta, Nome_a FROM Partita";
...
<body background='./img/Coppa-Italia-2019.jpg'>
	...
	<!-- /Header -->
	<center> <h2 style="color:white">Partite</h2> </center>
	<?php
		if(isset($message))
	...
	while($riga = $sqlresult->fetch_assoc())
	{
		...
		"<tr>
			...
			<td> ".$riga['Data']." </td>
			<td> ".$riga['Sq_casa']." </td>
			<td> ".$riga['Sq_trasferta']." </td>
			<td> ".$riga['Nome_a']." </td>
            <td> <form action='ServerSQL.php' method='POST'> <input type='hidden' name='codGame' value='".$riga['Num_partita']."/".$riga['Nome_turno']."'./> <input type='submit' onclick='return ConfirmDelete();' name='btnDelGame' value='Elimina'/></form></td>
		</tr>";
	}
	echo 
	"					</tbody>
    ...
    {
      var x = confirm('Eliminare la partita?');
      if (x)
          return true;
      else
        return false;
