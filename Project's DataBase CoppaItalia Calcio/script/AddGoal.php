<?php 
	...
	$sqlPlayers = "SELECT Nome, Cognome FROM Giocatore";
	$sqlPlayerSurnameResult = @$conn->query($sqlPlayers);
	$sqlPlayerNameResult = @$conn->query($sqlPlayers);
?> 
...
<body background="./img/Coppa-Italia-2019.jpg">
	...
	<center> <h2 style="color:white">Nuovo Goal</h2> </center>
		<table class="form-style-5">
			<td>
			<div class="form-style-5">
				<form action="ServerSQL.php" method="POST">
					<fieldset>
						<legend><span class="number">*</span> Info Goal </legend>
						...
						<input type="number" name="minute" placeholder="Minuto..." required>
						<select name="surname_g">
						<?php 
							while($surname = $sqlPlayerSurnameResult->fetch_assoc())
							{
								echo "<option value=".$surname['Cognome'].">".$surname['Cognome']."</option>";
							}
						?>
						</select>
						<select name="name_g">
						<?php 
							while($name = $sqlPlayerNameResult->fetch_assoc())
							{
				...
				<input type="submit" name="btnAddGoal" value="Aggiungi" />
				</form>
			</div>
			</td>
		</table>
